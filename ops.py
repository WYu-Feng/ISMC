    ...
    with tf.variable_scope(scope):
        if pad > 0 :
            if (kernel - stride) % 2 == 0:
                ...
                pad_top = pad#1
                pad_bottom = kernel - stride - pad_top#2
                pad_left = pad#1
...
def resblock(x_init, channels, use_bias=True, scope='resblock_0'):
    #通过组合图像的不相近特征，将图像在DA域中的特征向量转换为DB域中的特征向量
    with tf.variable_scope(scope):
...
def adaptive_instance_layer_norm(x, gamma, beta, smoothing=True, scope='instance_layer_norm') :
    with tf.variable_scope(scope):
        ...
        #得到x数组中，第1,2维（图像维）的均值，方差
        x_ins = (x - ins_mean) / (tf.sqrt(ins_sigma + eps))
        #eps是一个很小的值，加到方差上，避免出现方差==0的情况
        ...
        #rho=(32, 32, 32, 64)
        #rho决定在AdaLIN规范化过程中，风格和内容各种所占的比例，比例参数可学习
        if smoothing :
            rho = tf.clip_by_value(rho - tf.constant(0.1), 0.0, 1.0)
            #将一个张量(rho - tf.constant(0.1))中的数值限制在一个范围之内
...
def cam_loss(source, non_source) :
    #B-A（此时应该有明显的关注点，关注点为1）
    #A-A（此时没有明显的关注点，所有为0）
    #−(Ex∼Xs[log(ηs(x))] + Ex∼Xt[log(1 − ηs(x))])
    ...
    #t到x
    #sigmoid_cross_entropy_with_logits 对于给定的logits计算和labels的sigmoid交叉熵
...
def discriminator_loss(loss_func, real, fake):
    ...
    for i in range(2) :
        ...
        if loss_func == 'lsgan' :
            ...
            #tf.square对a里的每一个元素求平方，可以看做求fake与0的差方
